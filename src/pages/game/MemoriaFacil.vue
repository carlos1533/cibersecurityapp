...
<template>
  <div class="container-fluid">
    <template v-if="!this.continue">
      <div class="header">
        <h1>Minijuego 1: Memoria</h1>
        <div>
          <span
            class="label"
          >Debes encontrar las parejas que te enseñarán a distinguir entre contraseñas buenas (color verde), contraseñas intermedias (color amarillo) y contraseñas malas (color rojo). Haz clic en cada carta para ver que esconden. ¿serás capaz de encontrar todas las parejas?</span>
    ...
    <template v-if="this.continue === true">
      ...
      <div class="splash" v-if="showSplash">
        ...
        <div class="content">
          ...
          <router-link :to="{ name: 'memoriaintermedio' }">
...
export default {
  ...
  methods: {
    ...
    finishGame: function finishGame() {
      this.started = false;
      this.setCustomerScore(this.score);
  ...
  created() {
    if (this.$store.getters.hasPlayedMemory) {
